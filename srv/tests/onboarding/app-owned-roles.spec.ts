...
describe('App-Owned Roles', () => {
    ...
    beforeAll(async () => {
        ...
        const subscriberAdminTokenResp = await tokenFixture.fetchPasswordGrantAccessToken(
            subscriberUserEmail,
    ...
    describe('role assignment via user_roles', () => {
        it('should assign app-owned roles that reference roles in the owner tenant (Req 1.1)', async () => {
            ...
            const tokenResp = await tokenFixture.fetchPasswordGrantAccessToken(
                subscriberUserEmail,
                subscriberUserPassword,
                subscriberDomain,
            );
    ...
    describe('/my/permissions policy resolution', () => {
        ...
        it('should return combined policies from tenant-local + app-owned roles (Req 3.2)', async () => {
            ...
            const freshTokenResp = await tokenFixture.fetchPasswordGrantAccessToken(
                subscriberUserEmail,
                subscriberUserPassword,
                subscriberDomain,
            );
    ...
    describe('unsubscribe behavior', () => {
        ...
        beforeAll(async () => {
            ...
            const subTokenResp = await tokenFixture.fetchPasswordGrantAccessToken(
        ...
        it('should remove user_roles for app-owned roles when unsubscribing (Req 1.4)', async () => {
            ...
            const tokenBefore = await tokenFixture.fetchPasswordGrantAccessToken(
                unsubSubscriberUserEmail,
                unsubSubscriberUserPassword,
                unsubSubscriberDomain,
            );
            ...
            const tokenAfter = await tokenFixture.fetchPasswordGrantAccessToken(
                unsubSubscriberUserEmail,
                unsubSubscriberUserPassword,
                unsubSubscriberDomain,
            );
