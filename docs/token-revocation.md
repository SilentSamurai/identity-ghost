...
Performs a full server-side logout sequence: revokes the refresh token family and instructs the client to clear session
cookies via `Set-Cookie` headers. This is a superset of `/revoke` — clients should prefer this endpoint on logout rather
than calling both.
...
The `Set-Cookie` headers instruct the browser to immediately expire all session cookies. These headers are always
present in a successful response, even when the token is invalid or already revoked.
