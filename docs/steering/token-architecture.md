...
- `ScopeResolverService.resolveScopes(requested, clientAllowed)` — two-way intersection, throws `invalid_scope` if
  empty.
...
- `TokenIssuanceService.issueToken(user, tenant, options)` — orchestrates scope resolution (two-way) and role fetching
  from DB, then passes both to auth service.
