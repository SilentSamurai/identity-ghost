...
**Current state**: `Tenant` entity serves as both tenant and OAuth client. It holds `clientId`, `clientSecret`, RSA
keys.
...
**Recommendation**: Option B. The `App` entity already exists and represents applications owned by tenants. A `Client`
entity per app makes the model cleaner and supports the real-world case where one tenant has a web app, a mobile app,
and a backend service — each needing different OAuth configs.
...
Note: `crypto.subtle` requires HTTPS or localhost. The comment says "requires HTTPS" — this is only true for production.
